...
class FASTQConverter:
    ...
    def convert(self, accession: str) -> FASTQConversionResult:
        ...
        try:
            ...
            files = self.get_fastq_paths(accession)
            return FASTQConversionResult(ConversionStatus.SUCCESS, files)
        
        except subprocess.CalledProcessError as e:
            self.logger.warning(f"FASTQ conversion failed for {accession}:\n{e.stderr}")
            return FASTQConversionResult(ConversionStatus.FAILED, [], e.stderr.strip())
